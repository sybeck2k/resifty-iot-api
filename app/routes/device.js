...
module.exports = function (server, config) {
  ...
  server.get(resource_base_url + '/:id/sensors', route_utils.validateClient, function (req, res, next) {
    Sensor.paginate({device: req.params.id, client: req.clientId}, req.page, req.results_per_page, function (err, page_count, resources) {
      if (err)
        return next(err);
